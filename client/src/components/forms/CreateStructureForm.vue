...
const emit = defineEmits(["closeModal", "itemCreated"]);
...
const handleSubmit = async () => {
  ...
  try {
    ...
    for (const parentId of selected.value) {
      try {
        await structureService.linkStructures(config.value.linkTable, {
          [config.value.linkFields.parent]: parentId,
          [config.value.linkFields.child]: createdStructure.id,
        });
      } catch (err) {
        console.error(
          `Failed to link ${config.value.parentType} ${parentId}:`,
          err
        );
      }
    }
